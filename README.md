...
If the the system requests more than 16,383 UUIDs at the same 100-nanosecond interval, an exception is thrown to conform the standard. In this implementation, the default strategy is to add a counter value to a timestamp value with millisecond accuracy. The clock sequence is forced to increment when the counter wraps around, after been incremented 10,000 times. When the clock sequence is incremented 16,383 times the exception is thrown. So it's necessary to generate 163,830,000 UUIDs in the same millisecond to throw an overrun exception, that is the maximum timestamp counter (10,000) multiplied by the maximum clock sequence (16,383).
