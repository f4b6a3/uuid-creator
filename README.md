...
The `uuidcreator.node` property is used by the `DefaultNodeIdentifierStrategy`. If this property or variable exists, its value is returned. Otherwise, a random value is returned. In the default strategy the `uuidcreator.node` property is a *preferred value* for the node identifier, that is, it overrides the random value generated by the node identifier strategy. It may be useful if you want to identify each single machine by yourself, instead of allowing the algorithm do it for you. It accepts a string in decimal or hexadecimal representation (Long.parse()).

The effective way to avoid collisions is to ensure that each generator has its own node identifier.

* Defining a system property:

```bash
// append to VM arguments
-Duuidcreator.node="281474976710"
```
...
DEPRECATION WARNING: The old property `uuidcreator.nodeid` and old variable `UUIDCREATOR_NODEID` are _deprecated_ because they only accept hexadecimal representation (without 0x).
...
In this library the node identifier is generated by a secure random generator by default. Alternatively you can trust a IEEE 802 MAC address or a system data hash as node identifier.

It's possible to define a _preferred node identifier_ by setting the system property `uuidcreator.node` or the environment variable`UUIDCREATOR_NODE`.

It's also possible to atribute a node identifier when instantiating a new generator. See it on the section [Configuration examples](#configuration-examples).

Again, the effective way to avoid collisions is to ensure that each generator has its own node identifier. As a suggestion, a device ID managed your the App can be used, if you don't want a node identifier generated from a random number, a MAC address or a system data hash.
...
```
 ...
 aaabbbba-aaaa-vaaa-dddd-eeeeeeeeeeee
