...
The `nodeid` property is used by `DefaultNodeIdentifierStrategy`. If this property or variable exists, it's value is returned. Otherwise, a system information hash is returned. I the default strategy the `nodeid` property is a *prefered value* for the node identifier, that is, it overrides the system information hash generated by the strategy. It may be useful if you want to identify each single machine by yourself, instead of allowing the algorithm do it for you. It accepts 12 hexadecimal chars. 
