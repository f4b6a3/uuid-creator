...
The `nodeid` property is used by `DefaultNodeIdentifierStrategy`. If this property or variable exists, it's value is returned. Otherwise, a system information hash is returned. I the default strategy the `nodeid` property is a *prefered value* for the node identifier, that is, it overrides the system information hash generated by the strategy. It may be useful if you want to identify each single machine by yourself, instead of allowing the algorithm do it for you.
...
The `state.directory` property is used by `AbstractTimeBasedUuidCreator`.  If this property or variable exists, it's value is returned. Otherwise, the default java directory for temporary files is returned. It may be used to set a different directory other than the "/tmp" directory on Linux, which may not keep the files after reboot.
