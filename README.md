...
* __ULID-based Guid:__ a GUID with 48 bits for Unix milliseconds and 80 bits for randomness <sup>[9]</sup>.
...
This implementation is derived from the ULID specification. The only difference is that the millisecond bits are moved to the end of the GUID. Both COMB Guid and ULID-based Guid use the same basic algorithm. See the section on ULID-based GUIDs.
...
### ULID-based GUID (non-standard)

This type of GUID is based on the ULID specification <sup>[9]</sup>. The first 48 bits represent the count of milliseconds since Unix Epoch, 1 January 1970. The remaining 60 bits are generated by a secure random number generator. 
...
```java
// ULID-based GUID
UUID uuid = UuidCreator.getUlidBasedGuid();
```

Examples of ULID-based GUID:
...
// with java random generator (java.util.Random)
UUID uuid = UuidCreator.getUlidBasedGuidCreator()
    .withRandomGenerator(new Random())
    .create();
